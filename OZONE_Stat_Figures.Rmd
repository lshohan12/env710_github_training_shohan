...
FL_summer_ozone_concen <- FL_summer_ozone_egrid %>%
 ...
 FL_summer_ozone_AQI <- FL_summer_ozone_egrid %>%
  ggplot(aes(x = Mean_Plant_NetGenMW, y = Average_Month_AQI_Value)) +
   geom_jitter() +
   #scale_x_log10() +
  labs(x = 'Average Nameplate Capacity (MW)', y = 'Average Ozone AQI',
       title = 'July ')
 FL_summer_ozone_AQI
```
```{r}
...
egrid_data_counties_all <- egrid_data %>%
  ...
  select(Plant.FIPS.county.code, 
         Plant.county.name,
         `Plant.nameplate.capacity.(MW)`, `Plant.state.abbreviation`) %>% #only important ones i think
  rename(County.FIPS.Code = Plant.FIPS.county.code) %>% #to merge egrid & ozone data frames by common col
...
all_summer_ozone_concen_fig <- summer_ozone_egrid_all %>%
  ggplot(aes(x = Mean_Plant_Nameplate_CapacityMW, y = average_month_ozone_concentration)) +
   geom_jitter() +
  scale_x_log10() +
  labs(x = 'Mean Nameplate Capacities of all Counties (MW)', y = 'Average Ozone Concentrations by County (unit)',
       title = 'Average Nameplate Capacities and \n Ozone Concentrations by County in July ')
all_summer_ozone_concen_fig
 ...
 all_summer_ozone_AQI <- summer_ozone_egrid_all %>%
  ggplot(aes(x = Mean_Plant_Nameplate_CapacityMW, y = Average_Month_AQI_Value)) +
   geom_jitter() +
   scale_x_log10() + #Heili suggested this!! better for outliers-can set ticks / break points on x axis too
  labs(x = 'Average County Nameplate Capacity (MW)', y = 'Average Ozone AQI',
       title = 'July ')
